...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "brcd",
		Short:             "BRC blockchain Daemon (server)",
	...
	// CLI command to print the brcd version
	rootCmd.AddCommand(version.Cmd())
	// CLI command to start and run the brcd (server)
	server.AddCommands(ctx, cdc, rootCmd, newApp, exportAppStateAndTMValidators)

	// prepare and add flags
	executor := cli.PrepareBaseCmd(rootCmd, "BRC", app.DefaultNodeHome)
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		gApp := app.NewBrcApp(logger, db, traceStore, false, uint(1))
	...
	gApp := app.NewBrcApp(logger, db, traceStore, true, uint(1))
	return gApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
